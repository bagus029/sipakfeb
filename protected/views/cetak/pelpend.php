...
<table border="1" width="100%" style="border-collapse: collapse;" >
	...
	<!--<tr>
        ...
        <?php
        $f=$x['BUKTI_PELPEND'];
        $ab="select count(BUKTI_PELPEND) AS B,BUKTI_PELPEND FROM unsur_pel_pendidikan where BUKTI_PELPEND='$f' and PENGAJUAN_KE='$pj'";
        $data2=Yii::app()->db->createCommand($ab)->queryAll();
        if ($unsur!=$x['SUBPELPEND'])
	{
		$no++;
		echo '<tr><td><b>'.$no.'.</b></td>
		<td colspan="5"><b>'.$x['SUBPELPEND'].'</b></td></tr>';
		$unsur=$x['SUBPELPEND'];
	} 
